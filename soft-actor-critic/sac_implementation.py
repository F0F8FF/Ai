    ...
    def __init__(
        ...
        if self.auto_entropy_tuning:
            if target_entropy is None:
                self.target_entropy = -float(action_dim)  # 기본값: -action_dim
            else:
...
def train_sac(env_name: str, episodes: int = 200, evaluate: bool = True):
...
def plot_training_results(rewards: List[float], eval_rewards: List[float], 
    ...
    # Plot rewards
    ax1.plot(rewards, label='Train Rewards')
    if len(eval_rewards) > 0:
        eval_x = np.linspace(0, len(rewards)-1, len(eval_rewards))
        ax1.plot(eval_x, eval_rewards, label='Eval Rewards')
    ...
    ax2.plot(alpha_losses, label='Alpha Loss')
...
if __name__ == "__main__":
    ...
    environments = [
        ...
        ("LunarLanderContinuous-v2", 500)
    ]
    
    for env_name, episodes in environments:
        print(f"\nTraining on {env_name}")
        rewards, eval_rewards, actor_losses, critic_losses, alpha_losses = train_sac(env_name, episodes)
